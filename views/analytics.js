...
window.Views.analytics = ({ db, App, setPrimary }) => {
  ...
  const goalProgresses = goals.map(g => window.Goals?.calcGoalProgress ? window.Goals.calcGoalProgress(g).final : 0);
  ...
  // Simple “bars” without any chart library
    ...
    <div class="stack">
      <div class="card big hero">
        ...
        <div>
          ...
          <div class="muted">Goals progress • Habits consistency • Budget snapshot</div>
      ...
      <div class="card big stack">
        ...
        <div class="grid">
          ...
          ${bar("Goals complete (share)", goals.length ? (goals.filter(g=> (window.Goals?.calcGoalProgress?.(g)?.final||0) >= 0.999).length/goals.length) : 0)}
          ${bar("Overdue goals (share)", goals.length ? (goals.filter(g=>window.Goals?.goalOverdue?.(g)).length/goals.length) : 0)}
        </div>
      </div>
      ...
      <div class="card big stack">
        ...
        <div class="grid">
          ...
          ${bar("Best streak (normalized)", (function(){
            ...
            return Math.min(1, best/60); // normalize to 60 days
          })())}
        </div>
      </div>
      ...
      <div class="card big stack">
        ...
        <div class="grid">
          <div class="card glass2 stack">
            ...
            <div class="kpi" style="font-size:22px">${Math.round(inc)} RON</div>
          </div>
          <div class="card glass2 stack">
            <div style="font-weight:900">Expense</div>
            <div class="kpi" style="font-size:22px">${Math.round(exp)} RON</div>
          </div>
          <div class="card glass2 stack">
            <div style="font-weight:900">Net</div>
            <div class="kpi" style="font-size:22px">${Math.round(net)} RON</div>
          </div>
        </div>
      </div>
    </div>
